#!/usr/bin/env bash

set -Eeuxo pipefail

mkdir -p /mnt/.cache/crows-nest/stern
mkdir -p /mnt/.cache/bowsprit
mkdir -p /mnt/.cache/hull
mkdir -p /mnt/.cache/bow/shark-infested-waters
mkdir -p /mnt/.cache/yardarm
mkdir -p /mnt/.cache/poop-deck
mkdir -p /mnt/.cache/stern
mkdir -p /mnt/.cache/helm
mkdir -p /mnt/.cache/ratlines
mkdir -p /mnt/.cache/hold
mkdir -p /mnt/.cache/forecastle/bunk
mkdir -p /mnt/.cache/forecastle/secret-compartment

cat <<EOF >"/mnt/.cache/crows-nest/shinny-object"
eW91LWxvb2stdXAtYXQtdGhlLXNoaW5ueS1vYmplY3QtYW5kLWEtc2VhZ3VsbC1hcHBlYXJzLW91dC1vZi1zdW5saWdodC1kcm9wcGluZy1hLWxpdHRsZS1wcmVzZW50LW9uLXlvdXItZmFjZS5UaGlzLWlzLW5vdC10aGUtcG9vcC1kZWNrLXNjcmFtLXlvdS1mbHlpbmctd3JldGNoIQo=
EOF

cat <<EOF >"/mnt/.cache/bow/shark-infested-waters/shark"
eW91LW11c3QtYmUtY3JhenktdG8tcGV0LWEtc2hhcmsteW91LWhhdmUtbG9zdC15b3VyLWhhbmQtYW5kLWhhdmUtYS1ob29rLW5vdwo=
EOF

cat <<EOF >"/mnt/.cache/hold/treasure-chest"
YS1tb25rZXktanVtcHMtb3V0LWFuZC1zY3JhdGNoZXMtZmFjZS1hbmQtYml0ZXMteW91ci1ub3NlLW91Y2ghCg==
EOF

cat <<EOF >"/mnt/.cache/forecastle/bunk/blanket"
d293LXRoZXJlLWFyZS1hLWxvdC1vZi1mbGVhcy1vbi10aGlzLWJsYW5rZXQtYW5kLWEtc291cC1zdGFpbi1hdC1sZWFzdC15b3UtaG9wZS1pdC1pcy1hLXNvdXAtc3RhaW4teXVrIQo=
EOF

cat <<EOF >"/mnt/.cache/crows-nest/stern/adventure-gallery"
eW91LWNhbi1zZWUtdGhlLWFkdmVudHVyZS1nYWxsZXJ5LWluLXRoZS1pbmRpYW4tb2NlYW4taWYteW91LWdldC1jbG9zZS1lbm91Z2gteW91LXByb2JhYmx5LWNvdWxkLXNpbmstaXQK
EOF

cat <<EOF >"/mnt/.cache/forecastle/secret-compartment/treasure-map-4"

EOF