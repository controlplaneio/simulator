...
resource "local_file" "admin_ssh_config" {
  ...
  filename        = format("%s/%s", var.admin_bundle_dir, local.ssh_config_filename)
  file_permission = "0600"
}

resource "local_file" "ansible_config" {
  content_base64 = module.cluster.ansible_config
  filename       = format("%s/%s", var.admin_bundle_dir, local.ansible_config_filename)
}

resource "local_file" "ansible_inventory" {
  content_base64 = module.cluster.ansible_inventory
  filename       = format("%s/%s", var.admin_bundle_dir, local.ansible_inventory_filename)
}
