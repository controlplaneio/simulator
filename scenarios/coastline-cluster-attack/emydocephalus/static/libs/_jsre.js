var $module = (function ($B) {
    ...
    MatchObject.group = function (self) {
      ...
      for (var i = 0, _len_i = arguments.length; i < _len_i; i++) {
        if (self.jsmatch[arguments[i]] === undefined) {
          res.push(_b_.None)
        } else {
          res.push(self.jsmatch[arguments[i]])
        }
      }
    ...
    MatchObject.groups = function (self, _default) {
      ...
      for (var i = 1, _len_i = self.length; i < _len_i; i++) {
        if (self.jsmatch[i] === undefined) {
          res.push(_default)
        } else {
          res.push(self.jsmatch[i])
        }
      }
      return _b_.tuple.$factory(res)
    }
    ...
    obj._is_valid = function (pattern) {
      if ($B.$options.re == 'pyre') {
        return false
      }  //force use of python's re module
      if ($B.$options.re == 'jsre') {
        return true
      }   //force use of brythons re module
      // FIXME: Improve
      ...
      if (!is_valid) {
        return false
      }  //if js won't parse the pattern return false
    ...
    obj.escape = function (string) {
      ...
      for (var i = 0, _len_i = string.length; i < _len_i; i++) {
        if (ok.search(string.charAt(i)) > -1) {
          res += string.charAt(i)
        }
      }
      return res
    ...
    obj.findall = function (pattern, string, flags) {
      ...
      if (args.length > 0) {
        ...
        var _flags = $B.$getattr($ns['kw'], 'get')('flags', 0)
      }
    ...
    obj.finditer = function (pattern, string, flags) {
      ...
      for (var j = 0, _len_j = jsmatch.length; j < _len_j; j++) {
        ...
        mo.group = function () {
          var res = []
          for (var i = 0, _len_i = arguments.length; i < _len_i; i++) {
            if (jsmatch[arguments[i]] === undefined) {
              res.push(_b_.None)
            } else {
              res.push(jsmatch[arguments[i]])
            }
          }
          if (arguments.length == 1) {
            return res[0]
        ...
        mo.start = function () {
          return mo._match.index
        }
        mo.end = function () {
          return mo._match.length - mo._match.index
        }
        mo.string = string
        _list.push(mo)
      }
    ...
    obj.search = function (pattern, string) {
      ...
      if (args.length > 0) {
        ...
        var flags = $B.$getattr($ns['kw'], 'get')('flags', '')
      }
      flags = normflags(flags)
      var jsp = new RegExp(pattern, flags)
    ...
    obj.sub = function (pattern, repl, string) {
      ...
      if (typeof repl == 'function') {
        return string.replace(jsp, $repl1)
      } else {
